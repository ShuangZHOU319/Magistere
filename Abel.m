...
for i = 1:num_perturbations
    u = perturbations(i, :)';
    F_u = FIM * u;
    abel_bound_i = (u' * u) / (u' * F_u);
    if abel_bound_i < abel_bound
        abel_bound = abel_bound_i;
    end
end
abel_bound = sqrt(abel_bound);

% Display results
disp(['Abel Bound: ', num2str(abel_bound)]);
